import { parentCoordinates } from '../utils/parentCoordinates';

export const commonPosition = (node) => {
    ...
    if (
        ...
        (node.width === node.parent.width - hPadding && node.height === node.parent.height - vPadding)
    ) {
        return '';
    }
    ...
    const centerX = Math.abs(2 * (node.x - parentX) + node.width - node.parent.width) < threshold;
    const centerY = Math.abs(2 * (node.y - parentY) + node.height - node.parent.height) < threshold;
...
export const getCommonPositionValue = (node) => {
...
export const commonIsAbsolutePosition = (node, optimizeLayout) => {
